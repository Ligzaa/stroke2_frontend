...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ต่อ URL แบบปลอดภัย ป้องกันช่องว่าง/เครื่องหมาย / เกิน
  const SUBMIT_URL = new URL('/api/submit', API_BASE).toString();
  ...
  // ---------- helper: ส่งผลไป backend ----------
  async function sendResult(payload) {
    console.log('POST ->', SUBMIT_URL, payload);
  ...
  function initBook() {
    ...
    // ป้องกันฟอร์มส่งเอง (เช่นกด Enter)
    ...
    // Option button (เลือกคำตอบในหน้าแบบประเมิน)
    bookContainer.addEventListener('click', function (e) {
      if (e.target.classList.contains('option-btn')) {
        ...
        // Save user answer
        userData.answers[pageIndex] = { question, answer: optionText, points };
  ...
  function updateSummary() {
    ...
    // === ส่งผลไปเก็บที่ backend ===
    const payload = {
      riskPercentage: String(riskLevel.percentage),
      gender: userData.gender,
      age: Number(userData.age)
    };
    sendResult(payload).catch(console.error);
  }
  ...
  function generateRecommendation(riskLevel, totalPoints) {
    ...
    // Check for smoking
    const smokingAnswer = Object.values(userData.answers).find(
      (a) => a.question.includes('สูบบุหรี่') && a.points > 0
    );
    if (smokingAnswer) {
      specificRecs += `
    ...
    // Check for alcohol
    const alcoholAnswer = Object.values(userData.answers).find(
      (a) => a.question.includes('แอลกอฮอล์') && a.points > 0
    );
    if (alcoholAnswer) {
      specificRecs += `
    ...
    const height = parseFloat(userData.height) / 100; // m
    if (weight && height) {
